...
function filterBySales(products, sales) {
  return sales
    ? products.filter((product) => product.salesStatus === true)
    : products;
}
  ...
  async function loadProducts(params = {}) {
    ...
    try {
      ...
      if (!getProductsResponse.ok)
        throw new Error(`HTTP error! status: ${getProductsResponse.status}`);
    ...
    filteredProducts =
      state.productBlock && state.productBlock !== 'all'
        ? products.data.filter((product) =>
            product.blocks.includes(state.productBlock),
          )
        : products.data;
    ...
    if (filteredProducts.length === 0) {
      showNotification(
        'No products found for the entered search term.',
        resetProducts,
      );
    ...
    for (const product of filteredProducts) {
      const cardFragment = await renderProductCard(
        product,
        template,
        onProductCardClick,
      );
